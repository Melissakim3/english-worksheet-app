...
1단계 - 지문 문장들 중 서술형 출제 가치가 높은 문장 2개 선정 (8~16단어 권장)
  ...
  ★ 세미콜론(;)이나 두 개 이상의 독립절이 접속사 없이 이어진 복잡한 겹문장은 선택하지 말 것
    (세미콜론이 있으면 그 앞/뒤 중 한쪽 독립절만 잘라서 하나의 완전한 문장으로 사용할 것)
...
5단계 - 출력 전 반드시 아래 순서로 검증할 것:
  (a) sourceSentence의 단어를 하나하나 세어서, wordBank의 모든 조각(원형으로 바뀐 것 포함)을 합쳤을 때
      빠짐없이 전부 포함되는지 단어 단위로 대조할 것 — 단 하나라도 빠지면 안 됨
  (b) answer 문장이 sourceSentence와 (형태 변형 적용 후) 정확히 일치하는지 확인
  (c) 위 (a)에서 누락된 단어가 발견되면 wordBank를 처음부터 다시 구성해서 빠진 단어가 없게 할 것
